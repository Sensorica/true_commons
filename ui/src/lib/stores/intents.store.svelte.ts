...
import { CREATE_INTENT, UPDATE_INTENT, DELETE_INTENT, PROPOSE_INTENT } from '../graphql/mutations';
...
function createIntentsStore(): IntentsStore {
	...
	async function validateIntentData(intent: IntentCreateParams): Promise<string[]> {
		...
		if (intent.effortQuantity?.hasUnit && !unitsStore.getUnitById(intent.effortQuantity.hasUnit)) {
	...
	async function updateIntent(id: string, intentData: IntentUpdateParams): Promise<Intent> {
		return withLoadingState(
			async () => {
				...
				intents = intents.map((intent) => (intent.id === id ? updatedIntent : intent));
	...
	function getUnsatisfiedIntents(): Intent[] {
		return intents.filter((intent) => !intent.satisfiedBy || intent.satisfiedBy.length === 0);
	}
	...
	function getIntentsForMatching(action?: string, resourceConformsTo?: string): Intent[] {
		return intents.filter((intent) => {
			...
			const resourceMatches =
				!resourceConformsTo || intent.resourceConformsTo?.id === resourceConformsTo;
	...
	async function proposeIntent(intentId: string, note?: string): Promise<Intent> {
		return withLoadingState(
			async () => {
				...
				intents = intents.map((intent) => (intent.id === intentId ? updatedIntent : intent));
	...
	function getIntentStatsByAgent(agentId: string) {
		...
		const satisfied = intents.filter(
			(intent) =>
				(intent.provider?.id === agentId || intent.receiver?.id === agentId) &&
				intent.satisfiedBy &&
				intent.satisfiedBy.length > 0
		);
		const unsatisfied = intents.filter(
			(intent) =>
				(intent.provider?.id === agentId || intent.receiver?.id === agentId) &&
