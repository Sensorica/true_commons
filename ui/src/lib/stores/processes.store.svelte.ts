...
export interface ProcessesStore {
	...
	getProcessWithEvents(
		id: string
	): Promise<Process & { inputs: EconomicEvent[]; outputs: EconomicEvent[] }>;
	validateProcessData(process: ProcessCreateParams): Promise<string[]>;
	getProcessesByAgent(agentId: string): Process[];
	getProcessStatsByAgent(agentId: string): {
		total: number;
		finished: number;
		inProgress: number;
	};
...
function createProcessesStore(): ProcessesStore {
	 ...
	 */
	async function getProcessWithEvents(
		id: string
	): Promise<Process & { inputs: EconomicEvent[]; outputs: EconomicEvent[] }> {
		return withLoadingState(
			async () => {
	...
	async function getProcessOutputs(id: string): Promise<EconomicEvent[]> {
		const process = await getProcessWithEvents(id);
		return process.outputs;
	}

	/**
	 * Filters processes that are within the scope of a specific agent.
	 */
	function getProcessesByAgent(agentId: string): Process[] {
		return processes.filter((process) => process.inScopeOf?.id === agentId);
	}

	/**
	 * Calculates statistics for an agent's processes.
	 */
	function getProcessStatsByAgent(agentId: string) {
		const agentProcesses = getProcessesByAgent(agentId);
		const finished = agentProcesses.filter((p) => p.isFinished).length;
		const inProgress = agentProcesses.length - finished;
