...
export interface CommitmentsStore {
	...
	satisfyIntent(
		commitmentId: string,
		intentId: string
	): Promise<{ commitment: Commitment; intent: Intent }>;
...
function createCommitmentsStore(): CommitmentsStore {
	...
	async function updateCommitment(
		id: string,
		commitmentData: CommitmentUpdateParams
	): Promise<Commitment> {
		return withLoadingState(
			async () => {
				if (!hreaService.isInitialized) {
	...
	function getUnfulfilledCommitments(): Commitment[] {
		return commitments.filter(
			(commitment) => !commitment.fulfilledBy || commitment.fulfilledBy.length === 0
		);
	}
