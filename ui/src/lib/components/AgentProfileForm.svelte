<script lang="ts">
	...
	$effect(() => {
		if (agent) {
			...
			if (parsedMetadata) {
				formData.useStructuredMetadata = true;
				metadata = { ...metadata, ...parsedMetadata };
			} else {
				formData.useStructuredMetadata = false;
	...
	function handleSubmit(event: Event) {
		...
		if (formData.useStructuredMetadata) {
			// Add timestamps
			...
			noteContent = stringifyAgentMetadata(metadata);
		} else {
			noteContent = metadata.description || '';
		}
...
>
	...
	>
		<div
			class="sticky top-0 border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-700 dark:bg-gray-800"
		>
			<h2 class="text-xl font-semibold text-gray-900 dark:text-white">
				{isEditing ? 'Edit Agent Profile' : 'Create Agent Profile'}
			</h2>
		</div>
		...
		<form onsubmit={handleSubmit} class="space-y-6 p-6">
			...
			<div class="space-y-4">
				...
				<div class="flex items-center">
					<input
						type="checkbox"
						id="useStructuredMetadata"
						bind:checked={formData.useStructuredMetadata}
						class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 dark:border-gray-600"
					/>
					<label
						for="useStructuredMetadata"
						class="ml-2 block text-sm text-gray-900 dark:text-white"
					>
						Use structured metadata (recommended for detailed profiles)
					</label>
				</div>
			</div>
				...
				<div class="space-y-6">
					...
					<div class="space-y-4">
						...
						<div>
							<label
								for="skills-input"
								class="block text-sm font-medium text-gray-700 dark:text-gray-300">Skills</label
							>
