<script lang="ts">
	...
	import type { CommitmentCreateParams } from '$lib/graphql/types';
	...
	async function loadRelatedData() {
		try {
			await Promise.all([processesStore.fetchAllProcesses(), intentsStore.fetchAllIntents()]);
		} catch (err) {
	...
	async function validateForm(): Promise<boolean> {
		...
		validationErrors = await commitmentsStore.validateCommitmentData(formData);
	...
	async function handleSubmit(event: Event) {
		...
		try {
			const createdCommitment = await commitmentsStore.createCommitment(formData);
			onCommitmentCreated(createdCommitment);
	...
	// Add/remove intents from satisfies array
	function toggleIntentSatisfaction(intentId: string) {
		if (formData.satisfies?.includes(intentId)) {
			formData.satisfies = formData.satisfies.filter((id) => id !== intentId);
		} else {
			formData.satisfies = [...(formData.satisfies || []), intentId];
		}
	}
</script>

<div class="rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
	<h2 class="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Create Commitment</h2>
			...
			<div class="flex">
				...
				<div class="ml-3">
					...
					<p class="mt-2 text-sm text-yellow-700 dark:text-yellow-300">
						...
						initialized before creating commitments.
					</p>
					<div class="mt-3">
						<button
	...
	<form onsubmit={handleSubmit} class="space-y-6">
				...
				<div class="max-h-40 space-y-2 overflow-y-auto">
					{#each intentsStore.intents as intent}
						<label class="flex items-center space-x-2">
							<input
								type="checkbox"
								checked={formData.satisfies?.includes(intent.id)}
								onchange={() => toggleIntentSatisfaction(intent.id)}
								disabled={!foundationReady}
								class="rounded border-gray-300 text-blue-600 focus:ring-blue-500 disabled:opacity-50 dark:border-gray-600 dark:bg-gray-700"
							/>
							<span class="text-sm text-gray-700 dark:text-gray-300">
								{intent.action.label} - {intent.note || 'No description'}
							</span>
						</label>
					{/each}
				</div>
		...
		<div class="flex justify-end space-x-4">
			<button
				...
				class="inline-flex justify-center rounded-md border border-transparent bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 dark:bg-blue-500 dark:hover:bg-blue-600"
			>
				{#if loading}
